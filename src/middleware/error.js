...
const error = async (error, req, res, next) => {
  ...
  // sync/engine.js throws and middleware/auth.js). Anything that reaches here without one is an
  ...
  // The one exception is bad data the database itself rejected — see util/httpStatus.js.
  res.status(httpStatusFor(error)).json(errorResponse)
}
